...
class GraphIncidentEdgeIterator(Generic[T], Iterator[GraphEdge[T] | Tuple[T, T]]):
    """Iterates over edges incident to a particular vertex."""

    def __init__(self, vertex: GraphVertex[T], *, reverse: bool = False, const: bool = False) -> None:
        self._vertex = vertex
        self._edges: List[GraphEdge[T]] = vertex.incident_edges()
        self._iterator = BidirectionalIterator(self._edges, reverse=reverse)
        self._const = const

    def __iter__(self) -> GraphIncidentEdgeIterator[T]:
        return self
    ...
    def clone(self) -> GraphIncidentEdgeIterator[T]:
        clone = GraphIncidentEdgeIterator(self._vertex, reverse=self._iterator.reverse_flag, const=self._const)
        clone._iterator.set_index(self._iterator.current_index())
        clone._iterator.set_last_index(self._iterator.last_index())
        return clone

    def reverse(self) -> GraphIncidentEdgeIterator[T]:
        return GraphIncidentEdgeIterator(self._vertex, reverse=not self._iterator.reverse_flag, const=self._const)
